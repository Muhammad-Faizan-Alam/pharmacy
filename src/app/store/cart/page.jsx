...
//   };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-green-50 py-10 px-2 md:px-0">
      <div className="max-w-5xl mx-auto bg-white rounded-2xl shadow-2xl p-6 md:p-10">
        <div className="flex items-center gap-3 mb-8">
          ...
          <h1 className="text-3xl font-bold text-blue-800 drop-shadow">Your Cart</h1>
        </div>
        {loading ? (
          ...
          <div className="text-center py-20">
            <img src="/asset/empty-cart.png" alt="Empty Cart" className="mx-auto w-40 h-40 opacity-80 mb-6" onError={e => e.target.style.display='none'} />
          ...
          <div className="flex flex-col lg:flex-row gap-8">
            ...
            <div className="flex-1">
                ...
                <table className="min-w-full divide-y divide-gray-200">
                  <thead>
                    <tr className="bg-blue-50">
                      <th className="px-4 py-3 text-left text-xs font-bold text-blue-700 uppercase tracking-wider">Product</th>
                      <th className="px-4 py-3 text-center text-xs font-bold text-blue-700 uppercase tracking-wider">Price</th>
                      <th className="px-4 py-3 text-center text-xs font-bold text-blue-700 uppercase tracking-wider">Quantity</th>
                      <th className="px-4 py-3 text-center text-xs font-bold text-blue-700 uppercase tracking-wider">Total</th>
                    ...
                    {cart.items.map(item => (
                      <tr key={item.product._id || item.product.id} className="hover:bg-blue-50 transition">
                        <td className="px-4 py-4 flex items-center gap-4">
                          <img src={item.product.img || item.product.imageUrl} alt={item.product.name} className="w-16 h-16 object-contain rounded shadow" />
                          <div>
                        ...
                        <td className="px-4 py-4 text-center text-green-700 font-bold">Rs. {item.price.toFixed(2)}</td>
                        <td className="px-4 py-4 text-center">
                          <div className="flex items-center justify-center gap-2">
                            <button
                              className="p-1 rounded bg-gray-200 hover:bg-gray-300 text-xl font-bold text-gray-700 disabled:opacity-50"
                              onClick={() => updateQuantity(item.product._id || item.product.id, item.quantity - 1)}
                              disabled={item.quantity <= 1 || updating}
                            >
                              <FiMinus />
                            </button>
                            <span className="text-lg font-semibold text-blue-700">{item.quantity}</span>
                            <button
                              className="p-1 rounded bg-gray-200 hover:bg-gray-300 text-xl font-bold text-gray-700 disabled:opacity-50"
                              onClick={() => updateQuantity(item.product._id || item.product.id, item.quantity + 1)}
                        ...
                        <td className="px-4 py-4 text-center font-bold text-green-700">Rs. {(item.price * item.quantity).toFixed(2)}</td>
                        <td className="px-4 py-4 text-center">
                          <button
              ...
              <div className="flex justify-between mt-6">
                {/* <button
                  className="px-4 py-2 bg-red-600 text-white rounded hover:bg-red-700 transition font-semibold shadow disabled:opacity-50"
                  onClick={clearCart}
                  disabled={updating}
                >
                  Clear Cart
                </button> */}
                <Link href="/" className="px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700 transition font-semibold shadow">
            ...
            {/* Cart Summary */}
            <div className="w-full lg:w-1/3 bg-blue-50 rounded-2xl shadow-lg p-6 flex flex-col justify-between">
              <div>
                ...
                <div className="flex justify-between mb-4 border-t pt-2">
                  ...
                  <span className="text-lg font-bold text-green-700">Rs. {cart.total.toFixed(2)}</span>
                </div>
